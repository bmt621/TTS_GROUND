...
class TransformerEncoderLayer(Module):
    ...
    def _sa_block(self, x: Tensor,
        ...
        x, self.weights = self.self_attn(x, x, x,
                           attn_mask=attn_mask,
...
class TransformerDecoderLayer(Module):
    ...
    def _mha_block(self, x: Tensor, mem: Tensor,
                   attn_mask: Optional[Tensor], key_padding_mask: Optional[Tensor], is_causal: bool = False) -> Tensor:
        x, self.weights = self.multihead_attn(x, mem, mem,
                                attn_mask=attn_mask,
                                key_padding_mask=key_padding_mask,
                                is_causal=is_causal,
                                )
...
"""if __name__ == "__main__":
    encoder =  TransformerEncoder(TransformerEncoderLayer(20,4,batch_first=True),num_layers=3)
    x = torch.randn(7,13,20)
    out = encoder(x)
    weights = encoder.all_weights

    print("output shape: ",out.shape)
    print("weigths len: ",len(weights))
    print("lets see third layer: ",weights[-1].shape)"""